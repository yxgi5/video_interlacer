...
if(!rstn_cnt[7])
    ...
    // count from 0 
    //color_cntr <= lv_r ? (lv_dly_1 == lv_r ? color_cntr+lv_r : 0 ): 0;
    // count from 1
...
//    ative_line_cnt <= fv_r ? (pixcnt ==h_total-1)?(ative_line_cnt<v_active? ative_line_cnt+fv_r : 0): (ative_line_cnt<v_active? ative_line_cnt:0) : 0;
//    ative_line_cnt <= fv_r ? ( fv_dly_1 == fv_r ? ( pixcnt == h_total-1 ? ative_line_cnt + fv_r : ative_line_cnt ) : 0 ) : 0;
//    ative_line_cnt <= ((linecnt>=12'd0) & (linecnt<v_active)) ? (pixcnt ==h_total-1)?ative_line_cnt+1: ative_line_cnt : 0;
    // count from 0 
    //ative_line_cnt <= fv_r ? (pixcnt_dly_2 ==h_total-1)?ative_line_cnt+(fv_dly_1&fv_dly_2): ative_line_cnt : 0;
    // count from 1 
...
//assign data = {data_o[23:19],data_o[15:10],data_o[7:3]};
